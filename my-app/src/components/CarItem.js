...
function CarItem({ car, onBook }) {
    ...
    useEffect(() => {
        // Отримуємо актуальну кількість з sessionStorage, якщо є
    ...
    const handleBooking = () => {
        ...
        if (isConflict) {
            setMessage('❌ Цей автомобіль вже заброньований на вибрані дати.');
            return;
        }
        ...
        // Оновлюємо доступні авто
        const cars = JSON.parse(sessionStorage.getItem('cars')) || [];
        const updatedCars = cars.map(c => {
            if (c.id === car.id) {
                return { ...c, availableUnits: c.availableUnits - 1 };
            }
            return c;
        });
        sessionStorage.setItem('cars', JSON.stringify(updatedCars));
        setAvailableUnits(prev => prev - 1); // Оновлюємо локально

        setMessage('✅ Бронювання успішне!');
        onBook?.(car.id); // Якщо треба оновити батьківський стан
    };
