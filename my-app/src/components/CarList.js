...
import CarGrid from './CarGrid';  // Make sure to import CarGrid correctly
...
function CarList() {
    ...
    const handleBook = (carId) => {
        ...
        if (!carItem) {
            console.error("Car item not found for id:", carId);
            return;
        }
        ...
        const messageElement = carItem.querySelector(".message");

        if (!startDateInput || !endDateInput || !messageElement) {
            console.error("Missing inputs or message element");
            return;
        }
        ...
        if (!startDate || !endDate) {
            messageElement.textContent = "Будь ласка, заповніть обидві дати.";
        ...
        const hasConflict = existingBookings.some((booking) => {
            if (booking.carName !== carName) return false;
        ...
        if (hasConflict) {
            messageElement.textContent =
                "Цей автомобіль вже заброньовано на обрані дати.";
            messageElement.style.color = "red";
            return;
        }
        ...
        // Переходимо на сторінку з бронюванням
    ...
    return (
        <section id="cars">
            ...
            {/* Pass handleBook as a prop to CarGrid */}
